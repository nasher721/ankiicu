...
import type { RecentActivity } from "@/hooks/use-dashboard-stats";
...
export default function DashboardPage() {
  ...
  const progressPercent =
    stats?.generationProgress && stats.generationProgress.totalQuestionsTarget > 0
      ? Math.min(
          100,
          Math.round(
            (stats.generationProgress.totalCardsGenerated /
              stats.generationProgress.totalQuestionsTarget) *
              100,
          ),
        )
      : 0;
                ...
                <div className="space-y-3">
                  {stats.cardsByChapter.map(({ chapter, count }) => (
                    <div key={chapter} className="flex items-center justify-between">
                      <span className="text-sm truncate max-w-[200px]" title={chapter}>
                        {chapter}
                      </span>
              ...
              <div className="space-y-4">
                {stats?.recentActivity && stats.recentActivity.length > 0 ? (
                  stats.recentActivity.map((activity) => (
                    // Stable key: type + timestamp is unique per activity entry
                    <div
    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1">
        <div className="flex items-center justify-between mb-1">
          <span className="text-xs truncate max-w-[150px]" title={chapter.label}>
            {chapter.label}
          </span>
